...
BEGIN
	RETURN QUERY 
	SELECT SUM( (s.FECREALIZACION - s.FECRECEPCION))/COUNT(s.IDSERVICIO) AS MediaDias, SUM(r.HORAS)/COUNT(s.IDSERVICIO) AS MediaHoras
FROM servicio s JOIN reparacion r ON s.IDSERVICIO = r.IDSERVICIO;
END;
/
